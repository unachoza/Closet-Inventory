...
// Tight flow: Photo → Category → Basics → Details
...
// jsdom doesn't implement the browser default of a submit-type button click
// triggering form submission, so submits go through the form's submit event
// directly (same workaround used elsewhere in this repo's test suite).
function submitForm() {
...
describe("MultiStepForm — skip & submit", () => {
	...
	it("submit auto-generates the item name from color/brand/category", () => {
		renderForm();
		advanceToBasics();
		fireEvent.click(screen.getByRole("checkbox", { name: "blue" })); // color pill
		clickNext();
		submitForm();
	...
	it("occasion supports multiple picks, stored comma-joined", () => {
		renderForm();
		advanceToBasics();
		clickNext();
		...
		submitForm();
		expect(mockAddItem.mock.calls[0][0].occasion).toBe("casual, everyday");
	});
});

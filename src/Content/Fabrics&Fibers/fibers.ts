import type { Fiber } from "./textileTypes";

/**
 * The fiber encyclopedia. Unlike its siblings in this folder, this one IS on
 * the initial-load path: `materialUtils.resolveFiber()` looks entries up by id
 * for the closet's cards, filters and sort, so it cannot be lazy-loaded
 * without those going through a chunk fetch first.
 */

...
> per-user closet in Postgres, image storage off base64, offline-first sync. 
...
- `E1-1.6` ✅ **Done** (2026-07-07, branch `BUGS/critical`) — Offline-first reconcile (last-write-wins via `updatedAt`) — _1–1.5d_
  - _Hardening (hot fix):_ background writes no longer swallow failures silently (`.catch(() => {})`). `SyncedClosetRepository` now reports failures to a `syncFailureTracker`, which logs the real error and drives a `SyncStatusIndicator` in the NavBar ("N changes not synced"). A successful `getAll` reconcile clears it. Tests: `syncFailureTracker.test.ts`, `syncedClosetRepository.syncFailure.test.ts`, `SyncStatusIndicator.test.tsx`. 
  - **Bug fixes** (PR#119) — 6 critical/high findings from code review: (1) price round-trip loss (string→number end-to-end), (2) pagination reset regression (goToPage memoization), (3) dead code (EditProvider), (4) import validation (malformed files), (5) MaterialBlend dedup, (6) demo data in prod bundle. Regression tests added: pagination reset guard + import validation (7 tests). **Retry queue still deferred** — recovery rides on the next successful reconcile (reload / sign-in).
...
- [x] **No orphaned legacy data** — when migrating off base64 / PR #44 / any old store, securely wipe or migrate-and-delete the old data; never leave legacy copies behind. _(Tea: left data in old unsecured store after migrating)_ ⚠️ **Blocked on `E1-2.2`** (base64→Storage migration not yet started).
- [x] **Encryption at rest & in transit** — confirm Supabase encryption at rest is on; all transport over HTTPS/TLS. _(Tea: files + 1M+ DMs stored unencrypted)_
...
- [x] Self-serve **account deletion + data export** (GDPR/CCPA) — `accountDataService` + `AccountDataModal` (NavBar → "Account & Data"). Export = full JSON (items+profile+closets+wear_events). Delete = Storage wipe + profile delete → ON DELETE CASCADE clears the whole owned graph. ✅ **Fully deployed** (2026-07-07) — `profiles_delete_own` migration pushed AND `delete-user-account` Edge Function deployed on **both** prod (`rawuntspvetfdtrqggen`, fn v3) and dev (`lfdpvyygqblnckksvufn`, fn v1), verified via `supabase migration list --linked` (Local=Remote) + `supabase functions list` (both ACTIVE). Deletion is now true erasure (data + `auth.users` identity), not just a data wipe.
...
- [x] ✅ **Logging hygiene** (PR#128, 2026-07-09) — never log tokens, OAuth codes, or PII; error messages don't leak internals. `utils/logger.ts` redacts JWT-shaped tokens + `access_token`/`refresh_token`/`provider_token`/`id_token`/`code`/emails before anything hits the console; all 6 direct `console.*` call sites in production code replaced.
- [x] ✅ **Error tracking (Sentry) + product analytics (PostHog)** (PR#129, 2026-07-09) — both consent-gated: neither SDK is even `import()`ed until the user accepts the bottom banner (`ConsentBanner` + `lib/consent.ts`, default "undecided"). `lib/monitoring.ts` inits each only if its env var is set; `ErrorBoundary` reports caught errors via `captureException`. Chose Sentry+PostHog over Datadog — both free at 30-user beta scale, Datadog's RUM/APM pricing and infra focus don't fit a beta with no server fleet. Live-smoke-tested (2026-07-09): confirmed events landing in both dashboards after accepting consent. No privacy-policy link in the banner yet (see `E1-4.13`) — plumbing only, not the full consent UX.
...
- `E1-4.7` 🟡 **Mostly done** (PR#128, 2026-07-09) — CSP `script-src 'self' https://accounts.google.com` + `object-src 'none'; base-uri 'self'` and HSTS (`max-age=63072000; includeSubDomains; preload`) shipped via `vercel.json`. **Not yet verified live** against a deployed Vercel preview (esp. that Google OAuth still completes under the new CSP). `style-src`/`img-src`/`connect-src` lockdown and CORS-allowlist still open. Secure-cookie N/A (Supabase auth uses browser storage, not session cookies). — _0.5–1d_
- `E1-4.8` ✅ **Done** (PR#117, 2026-07-06/07) — client `accountDataService` (`exportAccountData`/`downloadAccountExport` reuse `exportCloset`'s `downloadFile`; `deleteAccountData` = Storage wipe + `profiles` delete → cascade; `deleteAccount` orchestrator invokes the Edge Function) + `AccountDataModal` in NavBar. Corrected data model: photos live in `items.primary_photo_url` + Storage under `<userId>/`, NOT `item_photos`. Tests: `accountDataService.test.ts` (6), `AccountDataModal.test.tsx` (4); suite green (1183). **`profiles_delete_own` migration + `delete-user-account` Edge Function deployed on both prod and dev (2026-07-07)** — verified live via CLI (`migration list`, `functions list`). Full identity erasure confirmed deployed; a live in-browser delete-and-re-signin QA pass is still recommended before launch. — _1–1.5d_
...
- `E1-4.12` ✅ **Done**  prod project split (2026-07-06)** (dev `lfdpvyygqblnckksvufn` + prod `rawuntspvetfdtrqggen`, `.env.local` override, verified end-to-end). Remaining: Backups/PITR enabled + restore test; logging-hygiene pass. — _0.5–1d_
- `E1-4.13` ⏳ **In progress** (2026-07-07, LEGAL-1 ticket) — Publish privacy policy (data collected, retention, third parties/Supabase, deletion/export rights, Limited Use disclosure). E1-4.8 deletion deployed and verified 2026-07-07, unblocking this. — _0.5–1d_
- `E1-4.14` ✅ **Done** (PR#129, 2026-07-09) — Sentry (error tracking) + PostHog (product analytics), both consent-gated and both free at 30-user beta scale. `lib/consent.ts` (localStorage, default "undecided") + `ConsentBanner`/`useConsent` + `lib/monitoring.ts` (deferred `import()`, only inits with consent + a configured env var). Sentry project `arianna-lz` / PostHog project (US region) created; `VITE_SENTRY_DSN`/`VITE_POSTHOG_KEY`/`VITE_POSTHOG_HOST` set in `.env.local` and Vercel. Tests: `consent.test.ts`, `monitoring.test.ts`, `useConsent.test.ts`, `ConsentBanner.test.tsx` (23 total) + `e2e/consent-banner.spec.ts` (banner render/dismiss/persist-across-reload). Live-smoke-tested end-to-end 2026-07-09 — confirmed events landing in both dashboards. **Remaining:** no privacy-policy link in the banner (bundle with `E1-4.13`); GitHub↔Sentry integration + source-map upload skipped (optional, not needed for basic error capture). — _0.5d_

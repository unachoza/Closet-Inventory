...
- `E3-bug.1` ⏳ **Email preview horizontal scroll** — some Gmail previews create an awkward horizontal scroll. Attempted `.gmail-container:has(.display-email-preview-panel){max-width:1175px}` previously; didn't hold. **2026-06-24 investigation (could NOT reproduce):** tried wide fixed-width tables + long unbroken tokens in subject/From across desktop + the split panel against a real browser (Playwright). No page-level overflow occurred, because `.gmail-preview-panel`, `.gmail-preview-body`, and `.gmail-email-list` all carry `overflow-y:auto`, which the CSS spec promotes to `overflow-x:auto` too — so each self-contains its overflow. ProductCard text is `nowrap`+ellipsis. Speculative `min-width:0` / `overflow-x` hardening was written then **reverted** as unverified (would be redundant given the auto-promotion). **Needs a concrete reproducing email/preview from the reporter** before a targeted fix (likely candidate if it recurs: an element _outside_ a scroll container — subject `h3` / From meta — needing `overflow-wrap:anywhere`). — _NOT FIXED · needs repro_

...
- `E3-bug.8` When toggling back and forth from email preview -> EditItemView some data is can be lost of user is mid flow. If on EditItemView starting to add details, wanting to go back and check the source email,clicking 'back to email' to verify something then import back to that item, any user added form info is gone
- `E3-bug.` When toggling back and forth from email preview -> EditItemView some data is can be lost of user is mid flow. If user has clicked include on a skipped item or include all, goes to the EditItemView form and toggles back to email button, the skipped/ included items reset
